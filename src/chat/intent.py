...
INTENTS = {
    # add 12.34 groceries optional description words on 2025-08-07 (allow optional negative for validation)
    "add_expense": re.compile(r"^(add|expense)\s+(?P<amount>-?\d+(?:\.\d{1,2})?)\s+(?P<category>\w+)(?:\s+(?P<desc>.*?))?(?:\s+on\s+(?P<date>\d{4}-\d{2}-\d{2}))?$", re.I),
    "set_budget": re.compile(r"^(?:set\s+budget|set)\s+(?P<category>\w+)\s+(?P<limit>-?\d+(?:\.\d{1,2})?)$", re.I),
    "show_budget": re.compile(r"^(budget)(?:\s+(?P<category>\w+))?$", re.I),
    "limits": re.compile(r"^(limits|list\s+limits)$", re.I),
    "export": re.compile(r"^(export)(?:\s+(?P<target>\w+))?(?:\s+(?P<path>.+))?$", re.I),
